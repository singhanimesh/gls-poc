...
function generateColors(startColor, endColor, count, type) {
	let sColor, eColor, colors = ["#ffffff",startColor],
...
function getColor(parentColor) {
	let color = colors[parentColor]; 	
	return color || '';  
};


const mapColorWithData = function (data , parentColor) {
	let items = data.data; 
	const endColor = getColor(parentColor);
	console.log(data);
